...
export const POST: RequestHandler = async ({ request, platform }) => {
  ...
  try {
    const { clerkId, email, name, localDate, timezoneOffset } = await request.json()
    ...
    // Get today's sets
    const todaySetsResult = await queries.getTodaySetsByUser(db, user.id, localDate, timezoneOffset)
