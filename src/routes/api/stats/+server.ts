...
export const GET: RequestHandler = async ({ url, platform }) => {
  ...
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      `).bind(parseInt(userId), dateStr).first<{ count: number }>();
    ...
    const cycleHistory = await db.prepare(`
...
async function calculateStreaks(db: D1Database, userId: number): Promise<{ currentStreak: number; bestStreak: number }> {
...
async function getWeeklyEvaluations(db: D1Database, userId: number): Promise<WeekEvaluation[]> {
  ...
  `).bind(userId).first<{ default_days_per_week: number }>();
