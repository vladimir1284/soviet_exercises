  ...
  <section>
    ...
    {#if $exercises.length === 0}
      ...
      <div class="card p-8 text-center">
        <div class="w-16 h-16 mx-auto mb-4 bg-surface-200/50 dark:bg-surface-200/30 rounded-2xl flex items-center justify-center">
...
<Modal 
  ...
  on:close={() => { showConfigureCycle = false; selectedExercise = null; }}
>
  {#if selectedExercise}
    <form class="space-y-6" on:submit|preventDefault={saveCycle}>
  ...
  <svelte:fragment slot="footer">
    <div class="flex gap-3">
      <button class="btn btn-secondary btn-md flex-1" on:click={() => { showConfigureCycle = false; selectedExercise = null; }}>
        {$_('common.cancel')}
      </button>
      <button 
