<script lang="ts">
  ...
  async function saveCycle() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        cycles.update((list) => [
          ...list.filter(
            (c) => c.exerciseId !== selectedExercise!.id || !c.isActive,
          ),
          cycle,
        ]);
  ...
  $: totalSets = Object.values($todayProgress).reduce(
    (sum, p) => sum + p.total,
    0,
  );
  $: completedSets = Object.values($todayProgress).reduce(
...
<div class="px-4 py-6 space-y-6">
  ...
  {#if totalSets > 0}
    <section class="card p-5 animate-fade-in">
      ...
      <!-- Progress bar -->
      <div
        class="h-3 bg-surface-200 dark:bg-surface-200 rounded-full overflow-hidden"
      >
        <div
      ...
      {#if completedSets >= totalSets}
        <p
          class="mt-3 text-center text-green-600 dark:text-green-400 font-medium"
        >
          {$_("log.allDone")}
        </p>
      {/if}
...
<!-- Add Exercise Modal -->
<Modal
  bind:open={showAddExercise}
  title={$_("exercises.add")}
  ...
  <svelte:fragment slot="footer">
    <div class="flex gap-3">
      <button
        ...
        on:click={() => (showAddExercise = false)}
      >
        {$_("common.cancel")}
      </button>
...
>
  {#if selectedExercise}
    <form class="space-y-4" on:submit|preventDefault={saveCycle}>
      <div
        class="flex items-center gap-4 p-4 bg-surface-100 dark:bg-surface-100/50 rounded-xl"
      >
        <div
      ...
      {#if cycleConfig.maxReps > 0}
        ...
        <div class="grid grid-cols-2 gap-4 text-center">
          <div class="p-3 bg-surface-100 dark:bg-surface-100/50 rounded-xl">
            <span
              class="num-display text-xl text-surface-900 dark:text-surface-900"
            >
  ...
  <svelte:fragment slot="footer">
    <div class="flex gap-3">
      ...
      <button
        ...
        on:click={saveCycle}
        disabled={cycleConfig.maxReps <= 0}
      >
