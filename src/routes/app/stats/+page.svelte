  ...
  {:else}
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="card p-4 animate-fade-in">
        <p
          class="text-xs text-surface-500 dark:text-surface-400 uppercase tracking-wide"
        >
          {$_("stats.totalSets")}
        </p>
        <p
          class="num-display text-3xl text-surface-900 dark:text-surface-900 mt-1"
        >
          ...
          {#each weekStats as day}
            ...
            <div class="flex-1 flex flex-col items-center gap-2">
              <div class="w-full flex flex-col items-center justify-end h-24">
                <span
                  class="text-xs font-medium text-surface-600 dark:text-surface-500 mb-1"
                >
                  {day.sets}
                </span>
                <div
    ...
    {#if cycleHistory.length > 1}
      ...
      {@const percentImprovement =
        firstMax > 0 ? ((improvement / firstMax) * 100).toFixed(1) : 0}
      <section class="card p-5 animate-fade-in" style="animation-delay: 0.3s;">
        ...
        <div class="flex items-center justify-center gap-8">
          <div class="text-center">
            ...
            <p
              class="num-display text-2xl text-surface-900 dark:text-surface-900"
            >
              {firstMax}
            </p>
          </div>
