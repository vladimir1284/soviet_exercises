...
export const queries = {
  ...
  createUser: (db: D1Database, clerkId: string, email?: string, name?: string) =>
    db.prepare('INSERT INTO users (clerk_id, email, name) VALUES (?, ?, ?) RETURNING *')
  ...
  updateUser: (db: D1Database, id: number, data: Partial<Pick<DbUser, 'email' | 'name' | 'locale' | 'theme'>>) => {
    ...
    return db.prepare(`UPDATE users SET ${setClause} WHERE id = ? RETURNING *`)
      .bind(...values, id)
      .first<DbUser>();
  },
  ...
  getExercisesByUser: (db: D1Database, userId: number) =>
    db.prepare('SELECT * FROM exercises WHERE user_id = ? ORDER BY sort_order, created_at')
      .bind(userId)
      .all<DbExercise>(),
  ...
  createExercise: (db: D1Database, userId: number, name: string, icon: string, color: string) =>
    db.prepare('INSERT INTO exercises (user_id, name, icon, color) VALUES (?, ?, ?, ?) RETURNING *')
      .bind(userId, name, icon, color)
      .first<DbExercise>(),
  
  updateExercise: (db: D1Database, id: number, data: Partial<Pick<DbExercise, 'name' | 'name_en' | 'icon' | 'color' | 'is_active' | 'sort_order'>>) => {
    const fields = Object.keys(data).filter(k => data[k as keyof typeof data] !== undefined);
    const values = fields.map(k => data[k as keyof typeof data]);
    const setClause = fields.map(f => `${f} = ?`).join(', ');
    return db.prepare(`UPDATE exercises SET ${setClause} WHERE id = ? RETURNING *`)
      .bind(...values, id)
      .first<DbExercise>();
  },
  
  deleteExercise: (db: D1Database, id: number) =>
    db.prepare('DELETE FROM exercises WHERE id = ?').bind(id).run(),
  
  // Cycles
  getCyclesByExercise: (db: D1Database, exerciseId: number) =>
    db.prepare('SELECT * FROM cycles WHERE exercise_id = ? ORDER BY created_at DESC')
      .bind(exerciseId)
      .all<DbCycle>(),
  ...
  createCycle: (db: D1Database, data: {
    exerciseId: number;
    maxReps: number;
    repsPerSet: number;
    setsPerDay: number;
  ...
  deactivateCycle: (db: D1Database, id: number) =>
    db.prepare('UPDATE cycles SET is_active = 0, end_date = DATE() WHERE id = ? RETURNING *')
      .bind(id)
      .first<DbCycle>(),
  
  // Sets
  getSetsByCycle: (db: D1Database, cycleId: number) =>
    db.prepare('SELECT * FROM sets WHERE cycle_id = ? ORDER BY completed_at DESC')
      .bind(cycleId)
      .all<DbSet>(),
  ...
  getSetsByDateRange: async (db: D1Database, userId: number, startDate: string, endDate: string) => {
    const result = await db.prepare(`
      ...
      ORDER BY s.completed_at DESC
    `).bind(userId, startDate, endDate).all<DbSet & { exercise_id: number }>();
    return result;
  },
  ...
  updateSet: (db: D1Database, id: number, data: { completedAt?: string; notes?: string }) =>
    db.prepare(`
      ...
      WHERE id = ? RETURNING *
    `).bind(data.completedAt || null, data.notes || null, id).first<DbSet>(),
  
  deleteSet: (db: D1Database, id: number) =>
    db.prepare('DELETE FROM sets WHERE id = ?').bind(id).run(),
  ...
  createSettings: (db: D1Database, userId: number) =>
    db.prepare('INSERT INTO user_settings (user_id) VALUES (?) RETURNING *')
      .bind(userId)
      .first<DbUserSettings>(),
  
  updateSettings: (db: D1Database, userId: number, data: Partial<Omit<DbUserSettings, 'id' | 'user_id' | 'created_at' | 'updated_at'>>) => {
    const fields = Object.keys(data).filter(k => data[k as keyof typeof data] !== undefined);
    const values = fields.map(k => data[k as keyof typeof data]);
    const setClause = fields.map(f => `${f} = ?`).join(', ');
    return db.prepare(`UPDATE user_settings SET ${setClause} WHERE user_id = ? RETURNING *`)
      .bind(...values, userId)
      .first<DbUserSettings>();
  },
  ...
  getTotalStats: async (db: D1Database, userId: number) => {
    const result = await db.prepare(`
      ...
      JOIN exercises e ON c.exercise_id = e.id
      WHERE e.user_id = ?
    `).bind(userId).first<{ total_sets: number; total_reps: number; active_days: number }>();
    return result;
  },
